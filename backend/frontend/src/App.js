...
const App = () => {
  ...
  const handleTagClick = async (tag) => {
    const response = await fetch(`http://localhost:8080/api/articles?tag=${tag}`); // `/api/articles?tag=${tag}`
  ...
  useEffect(() => {
    const fetchLatestIds = async () => {
      const response = await fetch('http://localhost:8080/api/articles/latest?count=10'); //'/api/articles/latest?count=10'
      const data = await response.json();
      setLatestIds(data);
    };
    fetchLatestIds();
